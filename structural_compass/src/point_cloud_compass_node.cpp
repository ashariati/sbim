...
typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::msg::PointCloud2, geometry_msgs::msg::PoseStamped> Policy;
...
public:
    PointCloudCompassNode() : Node("point_cloud_compass_node"),
                              ...
                              pc_sync_(Policy(5), pc_sub_, pose_sub_) {
        ...
        pc_sync_.registerCallback(std::bind(&PointCloudCompassNode::callback, this, std::placeholders::_1,
                                            std::placeholders::_2));
        ...
        pc_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("transformed_scan", 100);

        RCLCPP_INFO(this->get_logger(), "PointCloudCompassNode initialized.");

        timer_ = this->create_wall_timer(std::chrono::duration<double>(1 / frequency_),
                                         std::bind(&PointCloudCompassNode::loop, this));
    }
    ...
    void callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg,
        ...
        message_queue_.emplace_back(pcl_cloud, *pose_msg);
